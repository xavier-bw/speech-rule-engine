 ...
 * Adapted for Dutch by Xavier Van Elsacker
 */
...
function hundredsToWords_(num: number): string {
  ...
  let ones = Math.floor(n / 100);
  let onestr = NUMBERS.ones[ones];
  str += ones ? (ones > 1 ? onestr : '') + 'honderd' : '';
  n = n % 100;
  if (n) {
    str += str ? NUMBERS.numSep : '';
    onestr = NUMBERS.ones[n];
    if (onestr) {
      str += (str && n < 13 ? ' en ' : '') + onestr;
    } else {
      const tens = NUMBERS.tens[Math.floor(n / 10)];
      onestr = NUMBERS.ones[n % 10];
...
function numberToOrdinal(num: number /*, plural: boolean*/): string {
  ...
  /* targeting math and science, you would not use these in that context
  if (num === 2) {
    return 'halve';
  }
  if (num === 4) {
